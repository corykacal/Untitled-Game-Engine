...
{
    Bind();
    GLCall( glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, m_IndexBuffer.size() * sizeof(m_IndexBuffer[0]), m_IndexBuffer.data()) );
    GLCall( glDrawElements(GL_TRIANGLES, m_IndexBuffer.size()*3, GL_UNSIGNED_INT, nullptr) );
}
...
void DynamicIndexBuffer::DeleteTriangles(vector<IndexTriangle*> triangles) {
